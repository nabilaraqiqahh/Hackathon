...
switch ($method) {
    ...
    case 'POST':
        ...
        if (isset($input['transaction_id'], $input['user_id'], $input['amount'])) {
            ...
            $query = "INSERT INTO payments (transaction_id, user_id, amount, payment_date, status) VALUES (?, ?, ?, ?, ?)";
            $success = executeAction($query, [$input['transaction_id'], $input['user_id'], $input['amount'], $payment_date, $status]);
